...
message_history = [
    ...
    {"role": "system", "content": "If user asks you to perform a task that you can't do or can write the code and want user to do it then just generate \"SAMi()\". ** You don't have to ask user to perform a task remember that ! **"},
    {"role": "system", "content": "** When the user asks for quick searching, playing music, games, scheduling an alarm and other tasks which cannot be performed by you and the user can only perform then also generate \'SAMi()\'**"},
